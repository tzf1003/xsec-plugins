...
sidecar、source gate、Codex review/已解决 threads（使用同一严格的官方 completed-summary 回退），只会向 Desktop 发送 Beta smoke。
Stable completion 已由其精确 Beta smoke callback 约束，因而在这个阶段明确不重复发送 Desktop 矩阵。
