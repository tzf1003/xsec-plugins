...
资产发现、攻击路径和系统终端通过 Rust stdio MCP 提供服务；源码仓库保留 `mcp.json`，平台二进制由构建生成。
...
从 Cargo 返回的实际产物路径读取文件。攻击路径和资产发现从 Desktop workspace 构建；系统终端从
自己的插件源码目录构建：

- `plugins/com.xsec.asset-discovery/bin/asset-discovery-mcp`
- `plugins/com.xsec.attack-path/bin/attack-path-mcp`
- `plugins/com.xsec.system-terminal/bin/system-terminal-mcp`

命令支持 Factory 已声明的 macOS 和 Linux 本机架构。每次执行都会调用 Cargo 增量构建；
编译失败即终止。所有产物均构建成功后才开始写入，每个文件以原子替换方式更新，并回读 SHA-256。
若写入阶段失败，命令会明确失败；修复报错后重新执行准备命令，确保所需文件均已准备完成。
...
资产发现前端另由该插件的 `pnpm build` 构建。MCP 准备命令仅负责上面的本机入口；
...
正式 Marketplace 发布由受保护的 Factory 流水线构建各平台产物：Desktop sidecar 绑定精确
Desktop revision，系统终端 sidecar 绑定登记源码仓库的 Beta revision。本地准备命令只更新开发源码目录。
