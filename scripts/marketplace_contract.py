...
def active_default_official_plugin_ids(root: Path) -> tuple[str, ...]:
    """Return the protected Registry's active first-party default set."""
    ...
    for index, entry in enumerate(entries):
        ...
        if policy != {"installation": "INSTALLED_BY_DEFAULT", "authentication": "ON_INSTALL"}:
            raise ValueError(f"active first-party Registry plugin {plugin_id} has an invalid default policy")
