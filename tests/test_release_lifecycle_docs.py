...
class ReleaseLifecycleDocumentationTests(unittest.TestCase):
    ...
    def test_final_merge_gate_is_trusted_revalidating_and_does_not_deadlock_normal_prs(self) -> None:
        ...
        self.assertIn("coderabbit_status_verified", final_merge)
        self.assertIn("requires a completed CodeRabbit review", final_merge)
        self.assertIn("reviews(first:100,after:$endCursor)", final_merge)
        self.assertNotIn('commits/${HEAD_SHA}/status', final_merge)
    ...
    def test_final_gate_requires_authenticated_coderabbit_evidence_and_resolved_threads(self) -> None:
        ...
        self.assertIn("coderabbit_status_verified", workflow)
        self.assertIn("requires a completed CodeRabbit review", workflow)
