...
class ReleaseLifecycleDocumentationTests(unittest.TestCase):
    ...
    def test_final_merge_gate_is_trusted_revalidating_and_does_not_deadlock_normal_prs(self) -> None:
        ...
        self.assertIn('issues/${PULL_NUMBER}/comments?per_page=100', final_merge)
        self.assertIn('.user.login == "coderabbitai[bot]"', final_merge)
        self.assertIn('<!-- recent_review_end -->', final_merge)
