...
class ReleaseLifecycleDocumentationTests(unittest.TestCase):
    ...
    def test_final_merge_gate_is_trusted_revalidating_and_does_not_deadlock_normal_prs(self) -> None:
        ...
        self.assertIn("requires a successful trusted CodeRabbit status", final_merge)
        self.assertIn('commits/${HEAD_SHA}/statuses?per_page=100', final_merge)
        self.assertIn('creator.login == "coderabbitai[bot]"', final_merge)
        self.assertIn('creator.type == "Bot"', final_merge)
        self.assertIn('sort_by(.created_at) | last | .state == "success"', final_merge)
        self.assertNotIn("coderabbit_status_verified", final_merge)
        self.assertNotIn("reviews(first:100,after:$endCursor)", final_merge)
    ...
    def test_final_gate_requires_authenticated_coderabbit_evidence_and_resolved_threads(self) -> None:
        workflow = FINAL_MERGE_WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("requires a successful trusted CodeRabbit status", workflow)
        ...
        self.assertNotIn("reviews(first:100,after:$endCursor)", workflow)
