...
class FactoryLayoutMigrationTests(unittest.TestCase):
    ...
    def test_final_gate_keeps_layout_signature_candidates_pending_until_revalidated(self) -> None:
        ...
        plan = (ROOT / "scripts/factory_layout_migration_plan.py").read_text(encoding="utf-8")
        ...
        self.assertIn("PR_BASE_SHA", arm)
        self.assertIn("previous_filename", arm)
        self.assertIn("git/trees/" + "$" + "{PR_BASE_SHA}?recursive=1", arm)
        self.assertIn("gitlink_changed", arm)
        self.assertIn('mode == "160000"', arm)
        self.assertIn("incomplete Git tree", arm)
        ...
        self.assertIn("invalid layout submodule source", final)
        self.assertIn("gitlink_revisions", verifier)
        self.assertIn("verify_support_hashes", plan)
