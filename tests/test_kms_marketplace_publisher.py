...
class KmsMarketplacePublisherTests(unittest.TestCase):
    ...
    def test_publish_workflow_requires_main_oidc_and_desktop_smoke_dispatch(self) -> None:
        workflow = (ROOT / ".github" / "workflows" / "publish.yml").read_text(encoding="utf-8")
        ...
        self.assertIn("Check out current Factory main after acquiring the publication slot", workflow)
        ...
        self.assertIn("Refuse to sign while this plugin has a generated Factory PR awaiting final merge", workflow)
        self.assertIn("Cannot completely inspect generated Factory PR", workflow)
        self.assertIn(".xsec-factory/official-status/", workflow)
        self.assertIn("A historical terminal candidate must not strand a newer", workflow)
        self.assertIn('if [ -z "$SMOKE_RUN_URL" ]; then', workflow)
        self.assertIn("A manual Stable recovery has no immutable Desktop smoke", workflow)
        self.assertIn('head_sha="$(printf \'%s\' "$candidate" | jq -er .head.sha)"', workflow)
        self.assertIn('and .publication.smokeRunUrl == $smoke_url', workflow)
        self.assertIn('and .publication.marketplaceRevision == $marketplace_revision', workflow)
        self.assertIn('and .source.stableSha == $source_sha', workflow)
    ...
    def test_reconcile_source_is_the_only_status_signer_and_runs_source_gate_after_proof(self) -> None:
        ...
        self.assertIn("Publisher receipt Factory revision is not retained by Factory main.", reconcile)
